...
class QuadFullHybridUncertain(QuadFullHybrid):
    def __init__(self, quad_params, nn_pool_size, device):
        super().__init__(quad_params, nn_pool_size, device)

        #network for mean pred, and var pred
        self.nn_model = nn.ModuleList([nn.Linear(17, 64),
                                       nn.Tanh(),
                                       nn.Linear(64, 13)])
        self.nn_var_model = nn.ModuleList([nn.Linear(17, 64),
                                       nn.Tanh(),
                                       nn.Linear(64, 13)])
        ...
        torch.nn.init.xavier_uniform_(self.nn_model[0].weight)
        torch.nn.init.xavier_uniform_(self.nn_model[2].weight)
        torch.nn.init.xavier_uniform_(self.nn_var_model[0].weight)
        torch.nn.init.xavier_uniform_(self.nn_var_model[2].weight)        


    def forward(self, z):
        #mean pred and variance pred from parent class
        y_mean = super().forward(z)
        bs = z.size()[0]
        z = z.squeeze(1)
        temp = z
        for i, l in enumerate(self.nn_var_model):
            temp = l(temp)

            #we get positive var by applying exp 
        y_var = F.exp(temp)

        #rteturn mean and var pred
        return torch.cat([y_mean, y_var.unsqueeze(1)], dim=2)

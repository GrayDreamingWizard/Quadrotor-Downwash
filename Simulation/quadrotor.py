...
class Quadrotor(object):
    ...
    def update(self, state, control, t_step, upper_cf_state=None, params = None):
        def s_dot_fn(t, s):
            return self._s_dot_fn(t, s, control, upper_cf_state, params)

        sol = scipy.integrate.solve_ivp(s_dot_fn, (0, t_step), pack_state(state), first_step=t_step)
        state = unpack_state(sol['y'][:, -1])
        return state

    def _s_dot_fn(self, t, s, control, upper_cf_state=None, params=None):
        s_dot = np.zeros((13,))
        s_dot[0:3] = s[3:6]

        #base dynamics edit
        s_dot[3:6] = np.array([0, 0, -9.81]) + (control['cmd_thrust'] * rotate_k(s[6:10]))/self.mass
        if upper_cf_state is not None and params is not None:
            lower_cf_state = unpack_state(s)
            rel_pos = upper_cf_state['x'] - lower_cf_state['x']
            horizontal_dist = np.linalg.norm(rel_pos[:2])
            vertical_dist = max(0.1, rel_pos[2])
            force_mag = params['strength'] * np.exp(-horizontal_dist**2 / params['width']**2) / vertical_dist
            downwash_force = np.array([
                params['lateral_coeff'] * rel_pos[0],
                params['lateral_coeff'] * rel_pos[1],
                -force_mag
                ])
            s_dot[3:6] += downwash_force/self.mass
